 ...
 * [MAX_CONTROLS] times the tallest child's preferred height plus the row [spacing], independent of
 * how many controls it actually holds. [layoutChildren] then splits the box height into
 * [MAX_CONTROLS] equal slots and places the controls in the top slots, each stretched to the full
 * slot - so a box with one or two controls keeps ribbon-sized rows and leaves the remaining slots
 * empty at the bottom. A `Region` child has its `maxWidth` / `maxHeight` widened so it can be
 * stretched to the slot. The stack is always [alignment]-aligned to the top-left and never centred
 * vertically in the content row.
 *
 ...
 * nested inside.
 */
class FXMenuGroupSmallBox() : FXMenuGroupBox() {

    /** Vertical gap between the stacked rows. Fixed - controls stack to the ribbon convention. */
    val spacing: Double = 1.0

    /** The stack is always top-left aligned, never centred vertically in the group's content row. */
    val alignment: Pos = Pos.TOP_LEFT
    ...
    /** Largest managed child preferred width plus the box's horizontal insets. */
